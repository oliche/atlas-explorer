...
class TreeView(QtWidgets.QWidget):
    """
    Simple window to display the brain regions hierarchy as a tree
    """
...
class SliceView(QtWidgets.QWidget):
    ...
    def mouseMoveEvent(self, scenepos):
        ...
        iw, ih, w, h, v, rid = self.ctrl.cursor2xyamp(qpoint)
        ...
        if rid is None:
            ...
            # ba.regions.compute_hierarchy()
            br = self.topview.ctrl.atlas.regions
            self.label_hierarchy.setText('\n'.join(br.acronym[np.unique(br.hierarchy[:, rid])]))
            self.label_region.setText(br.name[rid])
            self.label_acronym.setText(br.acronym[rid])
...
class SliceController(PgImageController):
    ...
    def cursor2xyamp(self, qpoint):
        ...
        try:
            # we use the private method here to get the index quickly
            rid = ba._get_mapping(mapping=mapping)[ba.label.flat[ba._lookup(xyz)]]
        except ValueError:
            rid = None
        return iw, ih, w, h, v, rid
